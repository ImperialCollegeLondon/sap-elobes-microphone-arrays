classdef OpenSphereArray < SphericalHarmonicSoundFieldArray
    % OpenSphereArray provides an abstract class for implementing arrays
    % centred on an open spherical baffle.
    % In most cases it would probably make more sense to simply define a
    % FreeFieldArray with sensors in a spherical layout, but this gives a
    % direct comparison with RigidSphereArray
...
%     end
    ...
    methods
        function[obj] = OpenSphereArray(radius)
            % OpenSphereArray encapsulates the analytical response of microphones
            % on the surface of, or surrounding, an open sphere
